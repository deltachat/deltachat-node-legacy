...
NAN_METHOD(mailbox_new) {
  info.GetReturnValue().Set(MrMailboxWrap::NewInstance());
}

NAN_METHOD(mailbox_set_config) {
...
NAN_METHOD(mailbox_create_contact) {
  ASSERT_UNWRAP(info[0], mailbox, MrMailboxWrap);
...
NAN_METHOD(mailbox_configure_and_connect) {
  ASSERT_UNWRAP(info[0], mailbox, MrMailboxWrap)
...
NAN_MODULE_INIT(InitAll) {
  MrMailboxWrap::Init();
  EXPORT_FUNCTION(mailbox_new);
  EXPORT_FUNCTION(mailbox_set_config);
  EXPORT_FUNCTION(mailbox_configure_and_connect);
}
